...
test('consulta no passado: folga de 4 h — quem está atrasado não ouve que perdeu o horário', () => {
  const agora = '2026-09-24T15:08';
  assert.equal(consultaNoPassado('2026-09-24T15:00', agora), false, '8 minutos atrasado ainda é a consulta de hoje');
  assert.equal(consultaNoPassado('2026-09-24T11:00', agora), true, '4 h depois, aí sim passou');
  assert.equal(consultaNoPassado('2026-09-24T11:09', agora), false, 'dentro da folga por um minuto');
});
...
test('agora local e consulta se combinam: a virada do dia não engana', () => {
  ...
  assert.equal(consultaNoPassado('2026-09-22T17:00', agora), true, 'consulta das 17h, já 23h: passou da folga');
