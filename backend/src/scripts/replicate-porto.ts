...
const SRC_SLUG = process.env.PORTO_SRC_SLUG || 'doutor-hernia-serra';
const DST_SLUG = 'doutor-hernia-porto';
const DST_NAME = 'Doutor Hérnia Porto';
const KOMMO_SUBDOMAIN = 'doutorherniaporto';

const CIDADE_ORIGEM = /Serra/g;
const CIDADE_DESTINO = 'Porto';
...
const REPLY_FIELD_ID = 2450698;
const PAUSED_FIELD_ID = 2450700;
const WON_STATUS_IDS = [142];
const ALLOWED_STATUS_IDS = [106286847, 106287563, 106287575, 110471683, 106816055, 106816067];
...
async function main() {
  const token = process.env.PORTO_KOMMO_TOKEN;
  const geminiKey = process.env.PORTO_GEMINI_KEY;
  const claudeKey = process.env.PORTO_ANTHROPIC_KEY;
  if (!token) throw new Error('Faltou PORTO_KOMMO_TOKEN no ambiente.');
  if (!geminiKey && !claudeKey)
    throw new Error('Faltou PORTO_ANTHROPIC_KEY ou PORTO_GEMINI_KEY no ambiente.');
  ...
  Object.assign(clone, {
    ...
    ...(claudeKey
      ? { anthropicApiKey: claudeKey, anthropicModel: process.env.PORTO_CLAUDE_MODEL || 'claude-sonnet-5' }
      : { googleApiKey: geminiKey, googleModel: process.env.PORTO_GEMINI_MODEL || 'gemini-2.5-flash' }),
    kommoReplyFieldId: Number(process.env.PORTO_REPLY_FIELD_ID || REPLY_FIELD_ID),
    kommoPausedFieldId: Number(process.env.PORTO_PAUSED_FIELD_ID || PAUSED_FIELD_ID),
    kommoWonStatusIds: WON_STATUS_IDS,
    kommoAllowedStatusIds: ALLOWED_STATUS_IDS,
  });

  if (process.env.PORTO_SALESBOT_ID) clone.kommoSalesbotId = Number(process.env.PORTO_SALESBOT_ID);
  ...
  console.log('   - Webhook add_message -> https://api-vps.doutordigitalconsultoria.com/webhooks/kommo/doutor-hernia-porto');
