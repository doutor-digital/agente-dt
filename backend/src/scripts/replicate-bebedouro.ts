...
const SRC_SLUG = process.env.BEBEDOURO_SRC_SLUG || "doutor-hernia-serra"
const DST_SLUG = "doutor-hernia-bebedouro"
const DST_NAME = "Doutor Hérnia Bebedouro"
const KOMMO_SUBDOMAIN = "bebedourosp"

const PIPELINE_COMERCIAL = 14322239
const REPLY_FIELD_ID     = 210798
const PAUSED_FIELD_ID    = 210800
const WON_STATUS_IDS     = [ 142 ]
const ALLOWED_STATUS_IDS = [
    110614747, // Etapa de leads de entrada
    110663439, // EM QUALIFICAÇÃO
    110663443, // AGENDADO
    110663447, // COMPARECEU
    110663451, // EM NEGOCIAÇÃO
    110663455, // RETORNO PÓS-TRATAMENTO
]

const MOVE_STAGE_MAP: Record<number, { statusId: number, label: string }> = {
    110153704: { statusId: 110663439, label: "EM QUALIFICAÇÃO" },
    110153716: { statusId: 110663451, label: "EM NEGOCIAÇÃO"   },
    143:       { statusId: 143,       label: "PERDIDO"         },
}

// A IA só move o lead para AGENDADO por aqui (renderPipelineIntents). Sem isto ela
// marca a consulta e o lead fica parado — foi o furo corrigido em 20/08 nas 8 clínicas.
const PIPELINE_INTENTS = { scheduled_meeting: 110663443 }
...
const NAO_COPIAR = new Set<string>([
    ...
    "clinicAddress", "pixKey", "pixHolder", "avgTicketBrl",
])

// Bebedouro ainda não mandou a FICHA DA UNIDADE — nada de preço/endereço/equipe
// herdado da Serra pode sobrar nos textos.
function limparTextos(texto: string): string {
    return texto
        .replace(/Doutor Hérnia Serra/g, "Doutor Hérnia Bebedouro")
        .replace(/unidade Serra/g, "unidade Bebedouro")
        .replace(/Imperatriz/g, "Bebedouro")
        .replace(/\bSerra\b/g, "Bebedouro")
...
async function main() {
    const token        = process.env.BEBEDOURO_KOMMO_TOKEN
    const anthropicKey = process.env.BEBEDOURO_ANTHROPIC_KEY
    if (!token) throw new Error("Faltou BEBEDOURO_KOMMO_TOKEN no ambiente.")
    if (!anthropicKey) throw new Error("Faltou BEBEDOURO_ANTHROPIC_KEY no ambiente.")

    const credenciais = {
        llmProvider: "anthropic", anthropicApiKey: anthropicKey,
        anthropicModel: process.env.BEBEDOURO_CLAUDE_MODEL || "claude-sonnet-5",
    }
    ...
    Object.assign(clone, {
        ...
        ...credenciais,
        kommoReplyFieldId:  Number(process.env.BEBEDOURO_REPLY_FIELD_ID  || REPLY_FIELD_ID),
        kommoPausedFieldId: Number(process.env.BEBEDOURO_PAUSED_FIELD_ID || PAUSED_FIELD_ID),
        kommoWonStatusIds:     WON_STATUS_IDS,
        kommoAllowedStatusIds: ALLOWED_STATUS_IDS,
        pipelineIntents:       PIPELINE_INTENTS,
    ...
    if (existente && existente.actions.length > 0) {
        console.log(`⛔ Bebedouro já existe com ${existente.actions.length} ações. Abortando pra não duplicar.`)
        return
    }
    ...
    console.log(`✅ pipelineIntents.scheduled_meeting = ${PIPELINE_INTENTS.scheduled_meeting} (AGENDADO desta conta).`)

    console.log("\n⚠️  Lembretes lado Kommo (não é banco):")
    console.log("   - As tags usadas pelas ações add_tag precisam EXISTIR na conta (string exata).")
    console.log("   - Webhook (quando o WhatsApp chegar): https://agente-vps.doutordigitalconsultoria.com/api/webhooks/doutor-hernia-bebedouro/kommo")
...
main()
    ...
    .catch(async (e) => {
        console.error("❌ replicate-bebedouro falhou:", e)
