...
const SRC_SLUG = process.env.RIOVERDE_SRC_SLUG || "doutor-hernia-serra"
const DST_SLUG = "doutor-hernia-rioverde"
const DST_NAME = "Doutor Hérnia Rio Verde"
const KOMMO_SUBDOMAIN = "doutorherniarioverde2026"

const PIPELINE_COMERCIAL = 14392763
const REPLY_FIELD_ID     = 428080
const PAUSED_FIELD_ID    = 428082
const WON_STATUS_IDS     = [ 142 ]
const ALLOWED_STATUS_IDS = [
    111168635, // Etapa de entrada
    111168639, // EM QUALIFICAÇÃO
    111168643, // AGENDADO
    111168647, // NÃO COMPARECEU
    111168651, // COMPARECEU
    111168655, // EM NEGOCIAÇÃO
    111168659, // RETORNO PÓS-TRATAMENTO
]

const MOVE_STAGE_MAP: Record<number, { statusId: number, label: string }> = {
    110153704: { statusId: 111168639, label: "EM QUALIFICAÇÃO" },
    110153716: { statusId: 111168655, label: "EM NEGOCIAÇÃO"   },
...
// Rio Verde ainda não mandou a FICHA DA UNIDADE — nada de preço/endereço/equipe
// herdado da Serra pode sobrar nos textos.
function limparTextos(texto: string): string {
    return texto
        .replace(/Doutor Hérnia Serra/g, "Doutor Hérnia Rio Verde")
        .replace(/unidade Serra/g, "unidade Rio Verde")
        .replace(/Imperatriz/g, "Rio Verde")
        .replace(/\bSerra\b/g, "Rio Verde")
...
async function main() {
    const token        = process.env.RIOVERDE_KOMMO_TOKEN
    const anthropicKey = process.env.RIOVERDE_ANTHROPIC_KEY
    if (!token) throw new Error("Faltou RIOVERDE_KOMMO_TOKEN no ambiente.")
    const credenciais = {
        llmProvider: "anthropic", anthropicApiKey: anthropicKey || null,
        anthropicModel: process.env.RIOVERDE_CLAUDE_MODEL || "claude-sonnet-5",
    }
    ...
    Object.assign(clone, {
        ...
        ...credenciais,
        kommoReplyFieldId:  Number(process.env.RIOVERDE_REPLY_FIELD_ID  || REPLY_FIELD_ID),
        kommoPausedFieldId: Number(process.env.RIOVERDE_PAUSED_FIELD_ID || PAUSED_FIELD_ID),
    ...
    console.log("   - Webhook (quando o WhatsApp chegar): https://agente-vps.doutordigitalconsultoria.com/api/webhooks/doutor-hernia-rioverde/kommo")
...
main()
    ...
    .catch(async (e) => {
        console.error("❌ replicate-rioverde falhou:", e)
